...
MONGO_SECRET=$(git ls-files | xargs grep -l "[REDACTED_MONGODB_CREDENTIALS]@" 2>/dev/null || true)
FIREBASE_SECRET=$(git ls-files | xargs grep -l "[REDACTED_FIREBASE_API_KEY]-Owheowo" 2>/dev/null || true)
...
MONGO_HISTORY=$(git log --all --source -S "[REDACTED_MONGODB_CREDENTIALS]@" --oneline 2>/dev/null | head -5 || true)
FIREBASE_HISTORY=$(git log --all --source -S "[REDACTED_FIREBASE_API_KEY]-Owheowo" --oneline 2>/dev/null | head -5 || true)
